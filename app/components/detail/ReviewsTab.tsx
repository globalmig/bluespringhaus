...
export default function ReviewsTab({ reviews, speakerId }: ReviewItemProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/speakers/review`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ rating, comment, speaker_id: speakerId }),
        credentials: "include",
      });
  ...
  const handleEditSubmit = async (reviewId: number) => {
    ...
    try {
      const res = await fetch(`/api/speakers/review/${reviewId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="bg-slate-100">
      <div className="w-full flex flex-col gap-4 pb-32  px-4 py-10 max-w-[1440px] mx-auto ">
        {/* 리뷰 작성 폼 */}
