...
export default function ResetPassword() {
  ...
  useEffect(() => {
    (async () => {
      if (err) {
        setMsg(errd || "링크가 만료되었거나 유효하지 않습니다. 다시 시도해주세요.");
        return;
      }
    ...
    return (
      <div className="max-w-sm mx-auto mt-24 space-y-3">
        ...
        <form onSubmit={onResend} className="space-y-2">
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full border rounded-md px-3 py-2" placeholder="가입한 이메일" required />
  ...
  return (
    <form onSubmit={onChangePw} className="max-w-sm mx-auto mt-24 space-y-3">
      ...
      <input type="password" disabled={!ready} value={pw} onChange={(e) => setPw(e.target.value)} className="w-full border rounded-md px-3 py-2" placeholder="새 비밀번호" />
      <input type="password" disabled={!ready} value={pw2} onChange={(e) => setPw2(e.target.value)} className="w-full border rounded-md px-3 py-2" placeholder="새 비밀번호 확인" />
      <button disabled={!ready} className="w-full rounded-md bg-zinc-900 text-white py-2">
        비밀번호 변경
      </button>
