...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { data: review, error: reviewError } = await supabase.from("reviews").select("*").eq("id", id).eq("user_id", user.id).maybeSingle();
  ...
  if (req.method === "PUT") {
    ...
    const { data: updatedReviews, error: fetchError } = await supabase.from("reviews").select("*").eq("speaker_id", review.speaker_id).order("created_at", { ascending: false });
  ...
  if (req.method === "DELETE") {
    const { error: deleteError } = await supabase.from("reviews").delete().eq("id", id).eq("user_id", user.id);
